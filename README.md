...
3. **Make the Gommage your ally** — Failure (erasure) isn't the end; it's a chance to accumulate Lumina. Consecutive failures trigger class-aware recovery: transient failures (timeout, rate limit, parse error) retry the same issue in-place with cooldown, while persistent failures (blocker, systematic) halt and escalate. Recovery preserves worktree progress and injects Lumina hints for the next attempt.
