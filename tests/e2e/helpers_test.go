...
// buildTestContainer starts a paintress test container once.
func buildTestContainer(t *testing.T, ctx context.Context) testcontainers.Container {
	t.Helper()
	req := testcontainers.ContainerRequest{
		Image: sharedImage,
		Cmd:   []string{"sleep", "infinity"},
		WaitingFor: wait.ForExec([]string{"paintress", "--version"}).
...
// runCmd executes paintress inside the test container.
func runCmd(t *testing.T, ctx context.Context, c testcontainers.Container, dir string, args ...string) (string, string, error) {
	t.Helper()
	fullCmd := []string{"sh", "-c", fmt.Sprintf("cd %s && /usr/local/bin/paintress %s", dir, strings.Join(args, " "))}
...
// runCmdStdin executes paintress inside the test container, piping data to stdin.
func runCmdStdin(t *testing.T, ctx context.Context, c testcontainers.Container, dir, stdin string, args ...string) (string, string, error) {
	t.Helper()
	fullCmd := []string{"sh", "-c", fmt.Sprintf("cat << 'EOF' | (cd %s && /usr/local/bin/paintress %s)\n%s\nEOF", dir, strings.Join(args, " "), stdin)}
	code, stdout, stderr := execInContainerWithExitCode(t, ctx, c, fullCmd)
	var err error
	if code != 0 {
		err = fmt.Errorf("exit code %d", code)
...
// initTestRepo creates a workspace inside the container, git init, and runs `paintress init`.
func initTestRepo(t *testing.T, ctx context.Context, c testcontainers.Container, dir string) {
	...
	execInContainer(t, ctx, c, []string{"sh", "-c", fmt.Sprintf("cd %s && paintress init --lang en .", dir)})
}
