...
func NewWorkspace(t *testing.T, level string) *Workspace {
	...
	w.overridePaintressClaudeCommand(t)
...
// overridePaintressClaudeCommand is a no-op verification that paintress can find
// the claude binary. Paintress resolves claude via PATH (DefaultClaudeCmd = "claude"
// in config.go), not via a config file field. TestMain prepends the binDir
// (containing our fake-claude) to PATH, so no config modification is needed.
// This function verifies the .expedition/config.yaml exists (init succeeded).
func (w *Workspace) overridePaintressClaudeCommand(t *testing.T) {
	t.Helper()
	// Verify .expedition/config.yaml exists (paintress init succeeded).
	cfgPath := filepath.Join(w.RepoPath, ".expedition", "config.yaml")
	if _, err := os.Stat(cfgPath); err != nil {
		t.Fatalf("paintress config not found at %s: %v", cfgPath, err)
	}
	// No modification needed: paintress discovers claude via PATH,
