...
func TestResolve_EnvVarHasPriority(t *testing.T) {
	...
	// when
	id, source := projectid.Resolve("/anywhere/projects/cwdguess/.expedition")
...
func TestResolve_FallsBackToCWDInference(t *testing.T) {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	cwd := filepath.Join(home, "projects", "foo", ".expedition", "outbox")
...
func TestResolve_RejectsCWDInferredInvalidID(t *testing.T) {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	cwd := filepath.Join(home, "projects", "with space", ".expedition")
...
func TestInjectProjectID_NoOpWhenUnresolved(t *testing.T) {
	// given — env empty + tmp HOME so cwd inference cannot match projects/<id>
	t.Setenv("RUNOPS_PROJECT_ID", "")
	home := t.TempDir()
