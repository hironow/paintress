...
func TestRecordMCPInvocation_IncrementsCounterAndHistogram(t *testing.T) {
	...
	platform.RecordMCPInvocation(ctx, "next_issue", "deprecated", 12*time.Millisecond)
...
func TestRecordMCPInvocation_AttributesIncludeToolNameAndStatus(t *testing.T) {
	...
	// when
	platform.RecordMCPInvocation(ctx, "update_gradient", "error", 2*time.Millisecond)
		...
		for _, m := range sm.Metrics {
			...
			for _, dp := range sum.DataPoints {
				for _, attr := range dp.Attributes.ToSlice() {
					if string(attr.Key) == "tool.name" && attr.Value.AsString() == "update_gradient" {
						foundTool = true
					}
	...
	if !foundTool {
		t.Error("expected tool.name=update_gradient attribute on metric data point")
	}
	if !foundStatus {
