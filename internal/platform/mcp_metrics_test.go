...
func TestRecordMCPInvocation_IncrementsCounterAndHistogram(t *testing.T) {
	...
	// when
	platform.RecordMCPInvocation(ctx, "paintress.ping", "ok", 5*time.Millisecond)
	platform.RecordMCPInvocation(ctx, "paintress.next_issue", "deprecated", 12*time.Millisecond)
	platform.RecordMCPInvocation(ctx, "paintress.ping", "ok", 3*time.Millisecond)
	...
	if count := histogramCount(t, rm, "mcp.tool.duration"); count != 3 {
...
func TestRecordMCPInvocation_AttributesIncludeToolNameAndStatus(t *testing.T) {
	...
	// when
	platform.RecordMCPInvocation(ctx, "paintress.update_gradient", "error", 2*time.Millisecond)
		...
		for _, m := range sm.Metrics {
			...
			for _, dp := range sum.DataPoints {
				for _, attr := range dp.Attributes.ToSlice() {
					if string(attr.Key) == "tool.name" && attr.Value.AsString() == "paintress.update_gradient" {
						foundTool = true
					}
	...
	if !foundTool {
		t.Error("expected tool.name=paintress.update_gradient attribute on metric data point")
	}
	if !foundStatus {
...
// histogramCount sums data point counts across the histogram metric of
// the given name. Local helper because sumCounter only handles Sum
// counters.
