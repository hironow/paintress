...
func TestExtractMeta_DmailWithFrontmatter(t *testing.T) {
	...
	if entry.Tool != "paintress" {
		t.Errorf("tool: got %q, want %q", entry.Tool, "paintress")
	}
...
func TestIndexWriter_Append_CreatesFile(t *testing.T) {
	...
	entries := []domain.IndexEntry{
		{Timestamp: "2026-03-10T14:30:00Z", Operation: "expedition", Issue: "ENG-1", Status: "success", Tool: "paintress", Path: "archive/report.md", Summary: "test"},
	}
...
func TestIndexWriter_Append_AppendsToExisting(t *testing.T) {
	...
	w := &session.IndexWriter{}
	e1 := []domain.IndexEntry{{Timestamp: "t1", Operation: "o1", Tool: "paintress", Path: "p1", Summary: "s1"}}
	e2 := []domain.IndexEntry{{Timestamp: "t2", Operation: "o2", Tool: "paintress", Path: "p2", Summary: "s2"}}
...
func TestIndexWriter_Append_ValidJSON(t *testing.T) {
	...
	w := &session.IndexWriter{}
	entries := []domain.IndexEntry{
		{Timestamp: "t1", Operation: "o1", Tool: "paintress", Path: "p1", Summary: "summary with \"quotes\""},
		{Timestamp: "t2", Operation: "o2", Tool: "paintress", Path: "p2", Summary: "s2"},
	}
...
func TestIndexWriter_Rebuild_ScansAllMd(t *testing.T) {
	...
	w := &session.IndexWriter{}
	n, err := w.Rebuild(indexPath, stateDir, "paintress")
	if err != nil {
...
func TestIndexWriter_Rebuild_OverwritesExisting(t *testing.T) {
	...
	w := &session.IndexWriter{}
	n, _ := w.Rebuild(indexPath, stateDir, "paintress")
	if n != 1 {
