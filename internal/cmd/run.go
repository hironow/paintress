...
func runExpedition(cmd *cobra.Command, args []string) error {
	...
	exitCode, ucErr := usecase.RunExpeditions(ctx, domain.NewRunExpeditionCommand(rp), p, eventStore, logger, notifier, &platform.OTelPolicyMetrics{}, session.NewInboxArchiver(nil), cfg.Continent, cfg.MaxRetries)
	if ucErr != nil {
