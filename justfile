...
test-package-audit:
    ...
    audit_dir() {
        local dir="$1" label="$2"
        ext=$(grep -rl '^package .*_test$' "$dir" --include='*_test.go' 2>/dev/null | grep -Fv '.expedition/' | xargs grep -c '^func Test' 2>/dev/null | awk -F: '{s+=$2}END{print s+0}')
        same=$(grep -rL '^package .*_test$' "$dir" --include='*_test.go' 2>/dev/null | grep -Fv '.expedition/' | xargs grep -c '^func Test' 2>/dev/null | awk -F: '{s+=$2}END{print s+0}')
        total=$((ext + same))
...
# Audit nosemgrep annotations: require [permanent] or [expires: YYYY-MM-DD] tag
nosemgrep-audit:
    #!/usr/bin/env bash
    set -euo pipefail
    rc=0
...
docs-check:
    ...
    @! grep -rn 'usecase は session' .semgrep/layers.yaml 2>/dev/null || (echo "ERROR: stale usecase->session allowance in semgrep" && exit 1)
...
test-package-rationale-audit:
    ...
    done < <(grep -rL '^package .*_test$' internal/ --include='*_test.go' 2>/dev/null | grep -Fv '.expedition/')
    if [ "$fail" -gt 0 ]; then
